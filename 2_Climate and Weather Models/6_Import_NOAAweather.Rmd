...
**Key paramaters:**
datasetid - see: http://www.ncdc.noaa.gov/cdo-web/dat
...
## 1. Find Weather Stations
 Data format note: No headers V1. Station ID; V2. Latitude; V3. Longitude; V4. Observation type; V5. Observation value; V6. Earliest date; V7. Latest Date. See http://www1.ncdc.noaa.gov/pub/data/ghcn/daily/readme.txt. A few key measurements in V4:PRCP - precipitation (1/10mm); SNWD - snow depth (mm); TMAX - max temp (1/10 deg C); TMIN - min temp (1/10 deg C); WESD - water equivalent of snow on ground (1/10mm)
```{r}
...
Note: Sometimes slow server speeds can result in downloads timing out. In case of this, try running the code again at a later time.
...
NOTE: Divide by 10 because measured in 1/10 degrees
...
for (year in 2008:2013){
  ...
  # Skip if data already cleaned
  if(!file.exists(GDFilePath)){  
    ...
    # Calculate growing degrees > 5 deg C
    WthrData$GD<-((WthrData$V4.TMAX/10+WthrData$V4.TMIN/10))/2-5 
...
## Explore weather station location data
```{r}
...
#7576 unique stations
length(unique(NOAAData$StationID))

# Count number of stations with no data
sum(is.na(NStations[,2])|NStations[,2]==0) 

# Count number of observations for each population and weather variable
...
# Total number of stations for which data is needed

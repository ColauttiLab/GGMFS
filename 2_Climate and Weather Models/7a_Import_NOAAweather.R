...
if(file.exists("WeatherRawData/NOAAStationData.csv")){  
  ...
  NOAAData<-{}
  Ndg<-5 # Range (+/- degrees lat/log) to search for weather stations
  ...
  for(i in 1:nrow(PopData)){
    ...
    CloseStations<-InData[InData$V2 > PopData$Latitude[i]-Ndg  & InData$V2 < PopData$Latitude[i]+Ndg &
                            InData$V3 > PopData$Longitude[i]-Ndg  & InData$V3 < PopData$Longitude[i]+Ndg &
                            InData$V5 < 2008 & InData$V6 > 2013,1:4]
...
for (year in 2008:2013){
  ...
  if(!file.exists(GDFilePath)){  # Skip if data already calculated
    ...
    WthrData<-WthrData[WthrData$V1 %in% NOAAData$StationID & WthrData$V3 %in% c("TMAX","TMIN","SNWD"),]
    ...
    WthrData$GD<-((WthrData$V4.TMAX/10+WthrData$V4.TMIN/10))/2-5 # Calculate growing degrees > 5 deg C
    WthrData$GD[!is.na(WthrData$GD) & WthrData$GD<0]<-0
    WthrData<-WthrData[,c("V1","V2","V4.SNWD","GD")]
    names(WthrData)<-c("StationID","Date","SNWD","GD")

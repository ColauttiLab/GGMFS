...
plothist<-function(xval,PData=PopMeans3,Lbls=F,FntSz=24,BWth=0.1,HJust=0,HLoc=0){
  
  # Calculate mean difference between regions, and permute P-values
  Boot<-regionbootstrap(PData,xval) 
  
  # basic formatting
  p<-ggplot(data=PData) + theme_classic(base_size=FntSz) + guides(fill=FALSE)  
  
  # T/F Print labels
  if(Lbls==F){p<-p + xlab("") + ylab("")
  } 
  ...
  if(Boot[1]>1000){Boot[1]<-format(Boot[1], big.mark=",", scientific=FALSE)} 
  ...
  # Add medians
  EUMed<-median(pull(PData[PData$Region=="Europe",xval]),na.rm=T)
  NAMed<-median(pull(PData[PData$Region=="NorthAm",xval]),na.rm=T)
